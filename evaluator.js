...
Evaluator.prototype.evaluate = function(node,level,graph){
	this.tempstack=[];
	...
	if(node.type==this.nodeTypes.READ){
		...
		if(v===undefined){
			this.memory.addVar(new Var(varName,resultType,resultValue,resultSymbol,this.level,conversions.codeToVarType(resultType)));
			graph.memoryChanged=true;
			return resultSymbol;
		}
		else{
